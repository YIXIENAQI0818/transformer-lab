...
严谨验证：喂同一段输入，比较两个模型的 logits：
  - 原始模型：byte-bpe/src/model.py 的 GPT（我们手写的纯现代骨架）
...
# import 原始模型（byte-bpe/src/model.py）
sys.path.insert(0, os.path.join(LAB_DIR, "byte-bpe", "src"))
