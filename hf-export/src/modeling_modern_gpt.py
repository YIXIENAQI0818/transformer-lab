...
但 byte-bpe 的纯现代骨架（RoPE / RMSNorm / SwiGLU / GQA / FlashAttention）不是标准架构，
transformers 里没有现成的类，所以要自己写一个 PreTrainedModel。

关键设计：**参数名与 byte-bpe/src/model.py 完全一致**（transformer.wte.weight、
...
结构（照搬 byte-bpe/src/model.py，去掉 KV cache 与朴素 toggle）：
...
# ---------------- RoPE（内联，照搬 byte-bpe/src/rope.py） ----------------
...
class ModernGPTConfig(PretrainedConfig):
    """纯现代骨架的 HF 配置类，字段对齐 byte-bpe 的 GPTConfig。"""
...
class CausalSelfAttention(nn.Module):
    """GQA + RoPE + FlashAttention（照搬 byte-bpe/src/model.py，去掉 KV cache）。"""
...
class ModernGPTForCausalLM(PreTrainedModel, GenerationMixin):
    """纯现代 decoder-only GPT 的 HF 版（参数名与 byte-bpe/src/model.py 对齐）。"""
