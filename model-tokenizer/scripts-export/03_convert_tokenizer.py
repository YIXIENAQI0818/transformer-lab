"""步骤 3：把 model-tokenizer 的 BPE tokenizer 迁成 HF 标准格式。

关键差异 vs char 时代：char tokenizer 不是标准，要写自定义 PreTrainedTokenizer + vocab.json；
而 model-tokenizer 用的是 tokenizers 库的 ByteLevel BPE（HF 原生格式），tokenizer 已经存成
...
def main():
    # 用 tokenizer_file 直接加载 model-tokenizer 训好的库版 BPE tokenizer
    tok = PreTrainedTokenizerFast(tokenizer_file=LIB_TOK_PATH)
    print(f"① 加载 model-tokenizer 的 BPE tokenizer：vocab_size = {tok.vocab_size}")
